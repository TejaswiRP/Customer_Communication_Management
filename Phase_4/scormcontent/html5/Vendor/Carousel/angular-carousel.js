...
(function () {
    ...
    // ========================================
    Carousel3dController.$inject = ['$scope', '$element', '$attrs', '$rootScope', '$timeout', '$log', 'PreloaderService'];

    function Carousel3dController($scope, $element, $attrs, $rootScope, $timeout, $log, PreloaderService) {
        var vm = this;
        $rootScope.carouselIndex = 0;
        vm.isLoading = true;
		...
		var newOpt = {border: 1, height: 270, perspective: 0, sourceProp: "src", space: 150,startSlide:0,visible:5,width:320};
		vm.carousel3dOptions = newOpt;
        angular.extend(vm.options, vm.carousel3dOptions);
		console.log(vm.carousel3dOptions)
                ...
                function handleResolve(imageLocations) {
                    ...
                    $timeout(function () {
						...
						if($rootScope.pageStatusList[$rootScope.currentTopic][$rootScope.currentPage]=="1"){
							setTimeout(function(){$(".page-text").find(".carousel-3d").find("img").parent().addClass("item-visited")},1000);
						}
                    });
                },
        ...
        function render(animate, speedTime) {
            ...
            var slideTop = (vm.options.topSpace == "auto") ? ((vm.options.height / 2) - (parseInt(vm.selectedSlide.attr('outerheight')) / 2)) : 0,
                slideLeft = ((vm.options.width / 2) - (parseInt(vm.selectedSlide.attr('outerwidth')) / 2))-8,
        ...
        function goSlide(index, motionless, farchange) {
            ...
            if (vm.slides[vm.currentIndex].visited == 1) {
                vm.slides[vm.currentIndex].addClass("item-visited")
            }
            vm.slides[vm.currentIndex].visited = 1;
            vm.selectedSlide = vm.slides[vm.currentIndex];
            vm.visited = [];
            for (var i = 0; i < vm.slides.length; i++) {
                vm.slides[i].removeClass('current');
                if (vm.slides[i].visited == 1)
                    vm.visited.push(1);
                else
                    vm.visited.push(0)
            }

           /*  if (vm.visited.indexOf(0) == -1){
                $rootScope.markVisitedPage();
				if($rootScope.CourseContent.nextPageAfterCompeltion && index == 0){
					$rootScope.currentPage++;
					$rootScope.loadScreen($rootScope.currentModule,$rootScope.currentTopic, $rootScope.currentPage);
				}
			} */
            ...
            $rootScope.carouselIndex = vm.currentIndex;
    ...
    function carousel3d($timeout) {
        ...
        var carousel3d = {
            ...
            '    <div class="carousal-back-btn" ng-click=\"vm.goNext()\"><img src="../course_images/pages/m01/t01/p04/back.png"/></div>' +
            '    <div class="left-mask"></div>' +
            '       <div class=\"carousel-3d\" ng-switch-when=\"true\" ng-show="vm.isRendered">' +
            '           <div ng-repeat=\"image in vm.ngModel track by $index\"><img  ng-src=\"{{image[vm.options.sourceProp]}}\"  class=\"slide-3d\" ng-click=\"vm.slideClicked($index)\" ng-swipe-left=\"vm.goPrev()\" ng-swipe-right=\"vm.goNext()\"></div>' +
            '       </div>' +
            '       <p ng-switch-when=\"false\" class="carousel-3d-loader-error">There was a problem during load</p>' +
            '    <div class="right-mask"></div>' +
            '    <div class="carousal-next-btn" ng-click=\"vm.goPrev()\"><img src="../course_images/pages/m01/t01/p04/next.png"/></div>' +
            '   </div>' +
